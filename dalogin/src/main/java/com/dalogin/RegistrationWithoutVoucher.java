...
public class RegistrationWithoutVoucher extends HttpServlet {
    ...
    */
    ...
    public synchronized void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        if (user.equals("") && user.length() > 0 && EmailValidator.validate(email)) {
        ...
        try {
			...
			if (hmac.equals(hmacHash) && ((T+T2) > System.currentTimeMillis())) {
              ...
              if ((new_hash_(pass, user, email, context) == "I") && SQLAccess.insert_device(deviceId, user, context)) {
				...
				if (ios != null) {
	        	   ...
	        	   // hmac error
	        	   response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Line 229");
	           }
