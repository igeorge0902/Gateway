...
class MenuVC: UIViewController {
    ...
    private func setupUI() {
        ...
        adminButton.frame.origin = CGPoint(x: view.frame.width * 0.15, y: (view.frame.height - 120) / 2)
    ...
    private func createImageView() -> UIImageView {
        let imageView = UIImageView(frame: CGRect(x: view.frame.width * 0.1, y: view.frame.height / 2, width: view.frame.width * 0.8, height: view.frame.height / 3))
    ...
    private func addData() {
        RestApiManager.sharedInstance.getRandomUser { [weak self] (json, error) in
