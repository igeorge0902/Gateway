...
class VenuesDetailsVC: UIViewController, UIScrollViewDelegate, UIPopoverPresentationControllerDelegate, UIGestureRecognizerDelegate, UIViewControllerTransitioningDelegate {
   ...
   // var popOverX:CGFloat!
    ...
    lazy var icons:Dictionary<String, String> = Dictionary()
    ...
    override func viewDidLoad() {
        ...
        var startx:CGFloat = self.view.frame.width * 0.1
        starty = self.view.frame.height * 0.15
        
        if imageWidth > self.view.frame.width {
            
            imageWidth = self.view.frame.width * 0.9
            imageHeight = imageWidth / aspectRatio
            startx = self.view.frame.width * 0.05
        }
        ...
        var startx_:CGFloat = self.view.frame.width * 0.1
        let starty_:CGFloat = self.view.frame.height * 0.85
        
        if imageWidth_ > self.view.frame.width {
            
            imageWidth_ = self.view.frame.width * 0.9
            imageHeight_ = imageWidth_ / aspectRatio_
            startx_ = self.view.frame.width * 0.05
        }
        ...
        scrollView.contentSize = CGSize(width: self.view.frame.width, height: self.view.frame.height * 0.8)
      ...
      //  btnData.addTarget(self, action: #selector(VenuesDetailsVC.dates), forControlEvents: UIControlEvents.TouchUpInside)
        ...
        let frame1 = CGRect(x: self.view.frame.width * 0.15, y: (starty*2.5)+imageHeight, width: 44, height: 20)
        ...
        let frame2 = CGRect(x: self.view.frame.width * 0.65, y: (starty*2.5)+imageHeight, width: 44, height: 20)
        ...
        let frameM = CGRect(x: self.view.frame.width * 0.35, y: (starty*2.5)+imageHeight, width: 44, height: 20)
        ...
        let frame3 = CGRect(x: self.view.frame.width * 0.75, y: (starty*2.9)+imageHeight, width: 44, height: 44)
        ...
        let frame3_ = CGRect(x: self.view.frame.width * 0.55, y: (starty*2.9)+imageHeight, width: 44, height: 44)
        ...
        let frame4 = CGRect(x: self.view.frame.width * 0.15, y: (starty*3)+imageHeight, width: 50, height: 30)
   ...
   //     buttonShare.addTarget(self, action: #selector(VenuesDetailsVC.shareOnFaceBook), for: UIControlEvents.touchUpInside)
        ...
        // create the textView
        nameTextView = UITextView(frame: CGRect(x: self.view.frame.size.height * 0.05, y: starty * 3.0, width: self.view.frame.size.width * 0.8, height: self.view.frame.height / 5))
 ...
 //       shareDialog.completion = { result in
  ...
  //      try! shareDialog.show()
   ...
   // }
    ...
    @objc func book(_ sender: UIButton, event: UIEvent) {
        ...
        if event.type == .touches {
            ...
            if let touch =  touches.first{
                ...
                popOverY = touch.location(in: self.scrollView).y
    ...
    @objc func map(_ sender:UIButton, event: UIEvent) {
        
        if AFNetworkReachabilityManager.shared().networkReachabilityStatus.rawValue != 0 {
            
            self.performSegue(withIdentifier: "goto_map2", sender: self)
            
        }
        
    }
    
    @objc func dates(_ sender: UIButton, event: UIEvent) {
    ...
    @objc func selectCalendar(_ sender: UIButton) {
